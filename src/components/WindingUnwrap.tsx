...
export function WindingUnwrap({ result }: Props) {
  ...
  return <section className="unwrap-panel" style={{ overflow: 'hidden' }}>
    ...
    <div className="unwrap-toolbar"><span>Нажмите катушку или паз — MotorGod покажет её путь и направление.</span>{(selected !== null || selectedSlot !== null) && <button className="secondary-button" onClick={() => { setSelected(null); setSelectedSlot(null) }}>Снять выбор</button>}</div>
      ...
      <div className="unwrap-canvas" style={{ position: 'relative', width, minWidth: width, minHeight: height }}>
        <svg className="unwrap-svg" style={{ display: 'block' }} width={width} height={height} viewBox={`0 0 ${width} ${height}`} aria-label="Схема укладки обмотки">
          ...
          {result.phaseGroups.map((group, groupIndex) => {
            ...
            return <g key={`${group.phase}${group.polarity}-${groupIndex}`} opacity={groupDimmed ? .18 : 1}><rect x="4" y={baseY - 12} width="26" height="22" rx="4" fill="#0b0f15" stroke={color} /><text x="17" y={baseY + 4} textAnchor="middle" fontSize="11" fontWeight="800" fill={color}>Г{groupIndex + 1}</text>{members.map((coil, localIndex) => { const x1 = left + (coil.sideA - 1) * slotGap, x2 = left + (coil.sideB - 1) * slotGap, y = baseY + localIndex * laneStep, color2 = phaseColors[coil.phase] ?? color, active = selected === coil.number || (selectedSlot !== null && (coil.sideA === selectedSlot || coil.sideB === selectedSlot)), lineWidth = active ? 5 : 3, labelX = (x1 + x2) / 2, forward = coil.sideB >= coil.sideA; return <g key={coil.number} onClick={() => selectCoil(coil.number)} opacity={active || (selected === null && selectedSlot === null) ? 1 : .18} style={{ cursor: 'pointer', touchAction: 'manipulation' }}><line x1={x1} y1={y - 14} x2={x1} y2={y} stroke={color2} strokeWidth={lineWidth} /><line x1={x1} y1={y} x2={x2} y2={y} stroke={color2} strokeWidth={lineWidth} /><line x1={x2} y1={y} x2={x2} y2={y + 14} stroke={color2} strokeWidth={lineWidth} /><polygon points={forward ? `${x2 - 9},${y - 5} ${x2},${y} ${x2 - 9},${y + 5}` : `${x2 + 9},${y - 5} ${x2},${y} ${x2 + 9},${y + 5}`} fill={color2} /><rect x={labelX - 15} y={y - 10} width="30" height="20" rx="3" fill="#0b0f15" stroke={active ? color2 : 'none'} /><text x={labelX} y={y + 5} textAnchor="middle" fontSize="12" fontWeight="800" fill={color2}>К{coil.number}</text></g> })}</g>
          })}
        </svg>
      </div>
    </div>
    <p className="diagram-note">Стрелка показывает направление от стороны A к стороне B. Нажмите К или паз для трассировки.</p>
  </section>
}
