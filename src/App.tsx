...
import { buildWindingModel } from './engine/windingModel'
...
function App() {
  ...
  const [schemaMode, setSchemaMode] = useState<SchemaMode>('simple')
  ...
  const windingModel = useMemo(() => winding ? buildWindingModel(winding) : null, [winding])
  ...
  function submit(event: React.FormEvent) {
    ...
    setError(''); setSubmitted(true); setViewMode('slots'); setSchemaMode('simple'); setSelectedCoil(null)
  }
  ...
  return <div className="app-shell">
    ...
    <main>
      ...
      <section className="calculator-card" id="calculator"><div className="section-heading"><div><div className="eyebrow">MOTOR ANALYZER</div><h2>Первичный анализ двигателя</h2></div><span className="status-dot">● CORE ONLINE</span></div>
        ...
        {winding && windingModel && <section className="winding-result"><div className="section-heading compact"><div><div className="eyebrow">WINDING ENGINE</div><h2>Расчётная модель обмотки</h2></div><span className="status-dot">{winding.windingType === 'integer-slot' ? 'INTEGER SLOT' : 'FRACTIONAL SLOT'}</span></div>
          ...
          <div className="schema-switch"><span>Представление схемы</span><button type="button" className={schemaMode === 'simple' ? 'active' : ''} onClick={() => setSchemaMode('simple')}>Простое</button><button type="button" className={schemaMode === 'full' ? 'active' : ''} onClick={() => setSchemaMode('full')}>Полное</button></div>
          {schemaMode === 'simple' ? <div className="simple-schema"><div className="schema-title">Упрощённая электрическая схема</div>{windingModel.phases.map((phase) => <div className="simple-phase" key={phase.phase}><div className={`phase-badge phase-${phase.phase.toLowerCase()}`}>{phase.phase}</div><span>Вход</span><strong>{phase.positive.map((number) => `К${number}`).join(' → ') || '—'}</strong><span>Выход</span></div>)}<small>Упрощённое представление групп. Фактическая последовательность соединения будет рассчитываться отдельным электрическим алгоритмом.</small></div> : <div className="full-schema"><div className="schema-title">Полная иерархическая модель</div><div className="schema-flow"><div className="schema-node">ПАЗЫ<br /><small>{winding.slotTable.length} физических пазов</small></div><span>↓</span><div className="schema-node">СТОРОНЫ КАТУШЕК<br /><small>2 слоя</small></div><span>↓</span><div className="schema-node">КАТУШКИ<br /><small>{winding.coils} элементов</small></div><span>↓</span><div className="schema-node">ФАЗНЫЕ ГРУППЫ<br /><small>6 зон</small></div><span>↓</span><div className="schema-node terminal">A / B / C<br /><small>электрические выводы — следующий этап</small></div></div><div className="group-mini-grid">{windingModel.sections.map((group) => <div className={`group-mini phase-border-${group.phase.toLowerCase()}`} key={group.id}><b>{group.phase}{group.polarity}</b><span>{group.coils.length} катушек</span><small>{group.coils.map((number) => `К${number}`).join(' · ') || 'пусто'}</small></div>)}</div></div>}
          ...
          <div className="winding-checks"><span>Автоматические проверки</span>{windingModel.checks.map((check) => <small key={check}>{check}</small>)}<small>⚠ Электрические соединения групп пока не рассчитываются автоматически</small></div>
          <p className="engineering-note">Полная модель является источником для упрощённой схемы. Перед практической перемоткой необходима инженерная проверка магнитных, фазовых и соединительных параметров.</p>
        </section>}
